...
describe("Velomark block rendering", () => {
  ...
  it("renders mermaid code blocks with a diagram shell", async () => {
    ...
    expect(mermaidBlock?.querySelector('[data-velomark-code-view-toggle]')).not.toBeNull();
  });

  it("toggles mermaid blocks between preview and source", async () => {
    const host = document.createElement("div");
    document.body.append(host);
    const svgPrototype = window.SVGElement.prototype as SVGElement & {
      getBBox?: () => DOMRect;
    ...
    const toggleButton = mermaidBlock?.querySelector(
  ...
  it("allows built-in mermaid preview toggles to be disabled", async () => {
    const host = document.createElement("div");
    document.body.append(host);
    const svgPrototype = window.SVGElement.prototype as SVGElement & {
      getBBox?: () => DOMRect;
    ...
    const dispose = render(
      () =>
        <Velomark
          codeBlockOptions={{ previewToggle: false }}
          markdown={"```mermaid\ngraph TD\nA-->B\n```"}
        />,
      host
    );
  ...
  it("allows built-in mermaid source view to be the default", async () => {
    const host = document.createElement("div");
    document.body.append(host);
    const svgPrototype = window.SVGElement.prototype as SVGElement & {
      getBBox?: () => DOMRect;
    ...
    const dispose = render(
      () =>
        <Velomark
          codeBlockOptions={{ defaultView: "source" }}
          markdown={"```mermaid\ngraph TD\nA-->B\n```"}
        />,
      host
    );
